---
title: "Compare under-sampling samplers"
...
source_sha256: 53afa5bf91b8d29de8ad7f000e7dda1ed78b72cdf07a0c620ed1a8215b755376
license: MIT
---

> [!note] Pinned original source
> This readable page was generated from the complete upstream file
> [`examples/under-sampling/plot_comparison_under_sampling.py`](https://github.com/scikit-learn-contrib/imbalanced-learn/blob/8504e95f0160f61d1b617ca66f779646d2ee609e/examples/under-sampling/plot_comparison_under_sampling.py) at commit
...
> `53afa5bf91b8d29de8ad7f000e7dda1ed78b72cdf07a0c620ed1a8215b755376`. The project is distributed under the
...
`make_classification` from scikit-learn but fixing
some parameters.
...
`ClusterCentroids` under-samples by replacing
...
`RandomUnderSampler` is the most naive way of
...
`NearMiss` algorithms implement some
...
`EditedNearestNeighbours` removes samples of
...
`AllKNN` is slightly different from the
`RepeatedEditedNearestNeighbours` by changing
...
`CondensedNearestNeighbour` makes use of a
...
`OneSidedSelection` also used the 1-NN and
use `TomekLinks` to remove the samples
considered noisy. The
`NeighbourhoodCleaningRule` use a
`EditedNearestNeighbours` to remove some
...
`InstanceHardnessThreshold` uses the
