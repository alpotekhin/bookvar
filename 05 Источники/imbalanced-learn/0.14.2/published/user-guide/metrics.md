...
source_sha256: a63ed81eb1690196cda691ec961341a61e4788e0d3bfe76486a38ae9bcdf6518
license: MIT
---

> [!note] Pinned original source
> This readable page was generated from the complete upstream file
> [`doc/metrics.rst`](https://github.com/scikit-learn-contrib/imbalanced-learn/blob/8504e95f0160f61d1b617ca66f779646d2ee609e/doc/metrics.rst) at commit
...
> `a63ed81eb1690196cda691ec961341a61e4788e0d3bfe76486a38ae9bcdf6518`. The project is distributed under the
...
imbalanced datasets. The module `metrics` offers a couple of
...
Currently, only the [precision and recall metrics](http://scikit-learn.org/stable/modules/generated/sklearn.metrics.precision_recall_fscore_support.html)
are implemented in scikit-learn. `sensitivity_specificity_support`,
`sensitivity_score`, and `specificity_score` add the possibility to
use those metrics.

#### Additional metrics specific to imbalanced datasets

The `geometric_mean_score`
[barandela2003strategies](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#barandela2003strategies) [kubat1997addressing](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#kubat1997addressing) is the root of the product
...
The `make_index_balanced_accuracy` [garcia2012effectiveness](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#garcia2012effectiveness) can
...
The `macro_averaged_mean_absolute_error` [esuli2009ordinal](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#esuli2009ordinal) is used
...
The `classification_report_imbalanced` will compute a set of metrics per
...
The `pairwise` submodule implements pairwise distances
that are available in scikit-learn while used in some of the methods in
imbalanced-learn.

#### Value Difference Metric

The class `ValueDifferenceMetric` is
implementing the Value Difference Metric proposed in
[stanfill1986toward](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#stanfill1986toward). This measure is used to compute the proximity
...
behaviour as given in [wilson1997improved](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#wilson1997improved). `X` will be represented by a
...
where $x$ and $y$ are two samples and $f$ a given
feature, $C$ is the number of classes, $p(c|x_{f})$ is the
conditional probability that the output class is $c$ given that
the feature value $f$ has the value $x$ and $k$ an
exponent usually defined to 1 or 2.

The distance for the feature vectors $X$ and $Y$ is
...
where $F$ is the number of feature and $r$ an exponent usually
