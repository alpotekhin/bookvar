...
source_sha256: c3ea4989fad8ecc8cbb28b3fd794877fbbbc5d3622063ce934beb20509409249
license: MIT
---

> [!note] Pinned original source
> This readable page was generated from the complete upstream file
> [`doc/over_sampling.rst`](https://github.com/scikit-learn-contrib/imbalanced-learn/blob/8504e95f0160f61d1b617ca66f779646d2ee609e/doc/over_sampling.rst) at commit
...
> `c3ea4989fad8ecc8cbb28b3fd794877fbbbc5d3622063ce934beb20509409249`. The project is distributed under the
...
You can refer to
[`sphx_glr_auto_examples_over-sampling_plot_comparison_over_sampling.py`](https://imbalanced-learn.org/stable/).
...
`RandomOverSampler` offers such scheme:
```python
...
[![Official imbalanced-learn figure: sphx_glr_plot_comparison_over_sampling_002.png](https://imbalanced-learn.org/stable/_images/sphx_glr_plot_comparison_over_sampling_002.png)](https://imbalanced-learn.org/stable/auto_examples/over-sampling/plot_comparison_over_sampling.html)
...
In addition, `RandomOverSampler` allows to sample heterogeneous data
...
(ROSE) [torelli2014rose](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#torelli2014rose).

[![Official imbalanced-learn figure: sphx_glr_plot_comparison_over_sampling_003.png](https://imbalanced-learn.org/stable/_images/sphx_glr_plot_comparison_over_sampling_003.png)](https://imbalanced-learn.org/stable/auto_examples/over-sampling/plot_comparison_over_sampling.html)
...
Technique (SMOTE) [chawla2002smote](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#chawla2002smote) and (ii) the Adaptive Synthetic
(ADASYN) [he2008adasyn](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#he2008adasyn) sampling method. These algorithms can be used in
the same manner:
```python
...
[![Official imbalanced-learn figure: sphx_glr_plot_comparison_over_sampling_004.png](https://imbalanced-learn.org/stable/_images/sphx_glr_plot_comparison_over_sampling_004.png)](https://imbalanced-learn.org/stable/auto_examples/over-sampling/plot_comparison_over_sampling.html)

### Ill-posed examples

While the `RandomOverSampler` is over-sampling by duplicating some of
the original samples of the minority class, `SMOTE` and `ADASYN`
generate new samples in by interpolation. However, the samples used to
interpolate/generate new synthetic samples differ. In fact, `ADASYN`
...
implementation of `SMOTE` will not make any distinction between easy and
...
[![Official imbalanced-learn figure: sphx_glr_plot_comparison_over_sampling_005.png](https://imbalanced-learn.org/stable/_images/sphx_glr_plot_comparison_over_sampling_005.png)](https://imbalanced-learn.org/stable/auto_examples/over-sampling/plot_comparison_over_sampling.html)

The sampling particularities of these two algorithms can lead to some peculiar
behavior as shown below.

[![Official imbalanced-learn figure: sphx_glr_plot_comparison_over_sampling_006.png](https://imbalanced-learn.org/stable/_images/sphx_glr_plot_comparison_over_sampling_006.png)](https://imbalanced-learn.org/stable/auto_examples/over-sampling/plot_comparison_over_sampling.html)
...
[![Official imbalanced-learn figure: sphx_glr_plot_comparison_over_sampling_007.png](https://imbalanced-learn.org/stable/_images/sphx_glr_plot_comparison_over_sampling_007.png)](https://imbalanced-learn.org/stable/auto_examples/over-sampling/plot_comparison_over_sampling.html)

The `BorderlineSMOTE` [han2005borderline](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#han2005borderline),
`SVMSMOTE` [nguyen2009borderline](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#nguyen2009borderline), and
`KMeansSMOTE` [last2017oversampling](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#last2017oversampling) offer some variant of the
SMOTE algorithm:
```python
...
none of the presented methods (apart of the class `RandomOverSampler`)
can deal with the categorical features. The `SMOTENC`
[chawla2002smote](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#chawla2002smote) is an extension of the `SMOTE` algorithm for
...
features. One needs to provide this information to `SMOTENC` via the
...
However, `SMOTENC` is only working when data is a mixed of numerical and
categorical features. If data are made of only categorical data, one can use
the `SMOTEN` variant [chawla2002smote](/sources/imbalanced-learn/0-14-2/user-guide/bibliography/#chawla2002smote). The algorithm changes in
two ways:
...
samples. Considering a sample $x_i$, a new sample $x_{new}$ will be
...
$x_{zi}$ as illustrated in the image below:

[![Official imbalanced-learn figure: sphx_glr_plot_illustration_generation_sample_001.png](https://imbalanced-learn.org/stable/_images/sphx_glr_plot_illustration_generation_sample_001.png)](https://imbalanced-learn.org/stable/auto_examples/over-sampling/plot_illustration_generation_sample.html)
...
samples $x_i$ ahead of generating the new samples.

The **regular** SMOTE algorithm --- cf. to the `SMOTE` object --- does not
impose any rule and will randomly pick-up all possible $x_i$ available.

The **borderline** SMOTE --- cf. to the `BorderlineSMOTE` with the
parameters `kind='borderline-1'` and `kind='borderline-2'` --- will
classify each sample $x_i$ to be (i) noise (i.e. all nearest-neighbors
are from a different class than the one of $x_i$), (ii) in danger
(i.e. at least half of the nearest neighbors are from the same class than
$x_i$, or (iii) safe (i.e. all nearest neighbors are from the same class
than $x_i$). **Borderline-1** and **Borderline-2** SMOTE will use the
samples *in danger* to generate new samples. In **Borderline-1** SMOTE,
$x_{zi}$ will belong to the same class than the one of the sample
$x_i$. On the contrary, **Borderline-2** SMOTE will consider
$x_{zi}$ which can be from any class.

**SVM** SMOTE --- cf. to `SVMSMOTE` --- uses an SVM classifier to find
...
**KMeans** SMOTE --- cf. to `KMeansSMOTE` --- uses a KMeans clustering
...
samples generated for each $x_i$ is proportional to the number of samples
which are not from the same class than $x_i$ in a given
...
equivalent to `k_neighbors` in `SMOTE`.

### Multi-class management

All algorithms can be used with multiple classes as well as binary classes
classification.  `RandomOverSampler` does not require any inter-class
information during the sample generation. Therefore, each targeted class is
resampled independently. In the contrary, both `ADASYN` and
`SMOTE` need information regarding the neighbourhood of each sample used
