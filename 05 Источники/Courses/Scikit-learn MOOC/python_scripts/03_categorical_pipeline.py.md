...
> This page preserves [`python_scripts/03_categorical_pipeline.py`](https://github.com/INRIA/scikit-learn-mooc/blob/0cc70c41c640578c28eb7fd38d8c1c19e09c8aa6/python_scripts/03_categorical_pipeline.py) from the
...
> [!note] `sparse_output=False` is used in the `OneHotEncoder` for didactic purposes,
...
> [!note] In general `OneHotEncoder` is the encoding strategy used when the
...
> [!tip] Be aware the `OrdinalEncoder` exposes a parameter also named `handle_unknown`.
...
> [!note] Here, we need to increase the maximum number of iterations to obtain a fully
