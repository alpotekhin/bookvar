...
> This page preserves [`python_scripts/ensemble_hyperparameters.py`](https://github.com/INRIA/scikit-learn-mooc/blob/0cc70c41c640578c28eb7fd38d8c1c19e09c8aa6/python_scripts/ensemble_hyperparameters.py) from the
...
> [!warning] For the sake of clarity, no nested cross-validation is used to estimate the
...
> [!warning] Tuning the `n_estimators` for random forests generally result in a waste of
...
> [!warning] Here, we tune `max_iter` but be aware that it is better to set `max_iter` to a
