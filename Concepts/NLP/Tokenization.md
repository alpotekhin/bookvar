...
```mermaid
flowchart LR
    A["Начальные символы"] --> B["Подсчёт соседних пар"]
    B --> C["Самая частая пара"]
    C --> D["Слияние в новый токен"]
    D --> E{"Словарь нужного размера?"}
    E -- "нет" --> B
    E -- "да" --> F["Список merge rules"]
```
*BPE tokenization: итеративное объединение наиболее частых пар символов/подслов до достижения целевого размера словаря (схема учебника)*
