...
export async function findBrokenBuiltLinks(distDir: string): Promise<string[]> {
  const pages = await Promise.all((await htmlFiles(distDir)).map(async (file): Promise<BuiltPage> => {
  ...
  for (const page of pages) {
    for (const href of attributeValues(page.html, 'href')) {
      if (/^(?:https?:|mailto:|tel:|javascript:)/i.test(href)) continue;
      const url = new URL(href, `https://handbook.invalid${page.route}`);
      if (/^\/(?:_astro|assets|pagefind)(?:\/|$)/.test(url.pathname)) continue;
...
if (process.argv[1] && resolve(process.argv[1]) === modulePath) {
  ...
  if (errors.length > 0) {
    ...
    console.log('Built link check passed: 0 broken internal routes or fragments.');
  }
}
