...
BatchNorm усредняет статистики по batch и пространственным позициям отдельно для
каждого канала. Она хорошо согласуется с CNN, но использует running statistics
на inference. LayerNorm нормализует признаки каждого примера независимо и
...
- нормы градиентов и updates;
- отношение $\|\Delta W\|/\|W\|$.

Плоский loss не сообщает, где исчез сигнал. Профиль по глубине показывает слой,
