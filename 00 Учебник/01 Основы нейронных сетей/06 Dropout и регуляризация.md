...
прямой проход различается между обучением и применением модели.
...
При применении модели маска не используется и дополнительного масштабирования не нужно.
...
При применении модели используется исходный $h$, без случайной маски. Это приближает
...
так: ошибка на обучающей выборке продолжает падать, ошибка на проверочной выборке достигает минимума и затем
...
Сначала нужно определить характер ошибки. Большой разрыв между обучением и проверкой
при хорошем результате на обучающей выборке указывает на высокую дисперсию: помогают дополнительные
...
- Inverted dropout сохраняет математическое ожидание и активен только при обучении.
