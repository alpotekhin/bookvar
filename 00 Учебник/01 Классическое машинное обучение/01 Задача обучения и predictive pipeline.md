...
В обучении с учителем есть таблица наблюдений, целевая величина и правило
...
из целевой среды. Поэтому модель, предварительная обработка, схема разбиения и метрика
...
не являются независимыми. Эта структура позже определит способ разбиения.

Столбцы делятся на:

- **признаки** $X$: сведения, доступные в момент предсказания;
- **целевая переменная** $y$: величина, которую требуется предсказать;
- идентификаторы и служебные поля, которые обычно нельзя подавать модели как
  обычные признаки.

Целевая переменная может быть непрерывной — тогда решается регрессия — или дискретной —
тогда классификация. Название алгоритма не определяет задачу. `LogisticRegression`
...
## Почему предварительная обработка входит в модель
...
Правильная единица оценки — весь `Pipeline`:
...
При каждом разбиении `Pipeline` заново оценивает `scaler` на обучающей части, затем
...
Обучающие данные изменяют параметры. Проверочные данные влияют на выбор модели,
гиперпараметров и порога. Тестовые данные используются после завершения выбора.
Если после каждого результата на тестовой выборке меняется код или модель, тестовая выборка превращается в
validation.

Случайное разбиение годится только тогда, когда строки действительно можно считать
обменными. Иначе применяется:

- групповое разбиение — все строки одного пациента или пользователя остаются вместе;
- временное разбиение — обучение на прошлом, проверка на будущем;
- стратифицированное разбиение — доля классов сохраняется в каждой части;
- заранее заданное разбиение — когда эксплуатационный сценарий задаёт фиксированную
  границу.

## Полный цикл

1. Определить объект предсказания и момент, когда признаки становятся доступны.
2. Зафиксировать целевую переменную и функцию потерь, отражающие требуемое решение.
3. Выбрать разбиение по структуре данных, а не по удобству.
4. Посчитать простую базовую модель.
5. Собрать предварительную обработку и estimator в один `Pipeline`.
6. Настраивать гиперпараметры только внутри процедуры обучения и проверки.
7. Один раз оценить выбранную процедуру на тестовой выборке.
8. Перед развёртыванием проверить, совпадают ли признаки и предварительная обработка в эксперименте и эксплуатации,
   а также сохраняется ли семантика целевой переменной.
...
непрерывную оценку, а правило принятия решения настраивается позднее на
отложенных данных.
...
корректный конвейер в эксперименте не гарантирует, что эксплуатационный процесс формирует те же значения.

Так конвейер предсказания становится воспроизводимым контрактом: схема входа,
...
- [[05 Источники/Courses/Scikit-learn MOOC/jupyter-book/predictive_modeling_pipeline/predictive_modeling_module_intro.md.md|Predictive modeling pipeline]] — полный модуль с упражнениями и решениями;
