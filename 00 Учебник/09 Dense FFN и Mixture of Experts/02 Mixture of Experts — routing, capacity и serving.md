...
Sparse MoE заменяет один dense FFN банком экспертов и выполняет лишь несколько
для каждого токена. Это разрывает привычную связь между общей ёмкостью и FLOPs
на токен, но не отменяет память weights и добавляет задачу назначения, сетевую
пересылку и дисбаланс.
