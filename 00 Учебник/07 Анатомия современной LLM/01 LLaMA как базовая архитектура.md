...
> LLaMA не изобрела «новый Transformer». Она собрала в чистой открытой модели
> удачный decoder-only recipe: pre-norm, RMSNorm, RoPE, SwiGLU и отсутствие
> лишних biases. Этот блок стал удобной точкой отсчёта: последующие Llama, Qwen,
> Mistral и многие другие модели меняют отдельные детали, но сохраняют его
> скелет.
