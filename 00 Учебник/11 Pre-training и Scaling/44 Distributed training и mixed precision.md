...
**Полный исполняемый модуль:** [[05 Источники/Courses/Harvard ML Systems/tinytorch/08_training|TinyTorch 08 — Training]]. Это однопроцессный эталон цикла обучения: `Trainer`, режимы train/eval, gradient clipping, accumulation, расписание learning rate и checkpointing помогают точно перечислить состояния, которые затем приходится распределять.
