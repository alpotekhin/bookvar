...
# MHA, MQA и GQA: почему queries можно не экономить, а KV — приходится

> [!abstract] Что нужно понять
> При autoregressive generation модель хранит keys и values всех прошлых
> токенов. MHA создаёт отдельные K/V для каждой head, MQA оставляет один общий
> набор, GQA делит query heads на группы. Меняется не базовая формула attention,
...
Старые $K_{1:S}$ и $V_{1:S}$ зависят только от уже обработанных tokens.
...
При следующем токене cache снова читается. Для длинного context и большого batch
это превращает decode в memory-bandwidth problem: accelerator тратит время не
только на arithmetic, но и на перенос K/V из HBM.
...
Это cache **одной** последовательности. Batch 16 потребовал бы примерно 64 GiB,
...
## MHA: отдельная память для каждой head
...
Head может выучить собственное пространство для поиска и собственное
представление переносимого content. Это наиболее свободный вариант.
...
Но все $k_r,v_r$ прошлого приходится хранить. В incremental decoding стоимость
такой свободы платится на каждом шаге.

## MQA: все queries читают одну K/V память

Multi-Query Attention оставляет много query heads, но использует общие K и V:
...
Query projections всё ещё различаются, поэтому heads могут задавать разные
вопросы. Но они задают их одной общей системе адресов K и одному набору values.

Noam Shazeer предложил MQA именно как способ уменьшить bandwidth incremental
decoding. Paper показывает практическое ускорение с небольшим, но возможным
ухудшением качества.

> [!note] Почему название multi-query
> «Multi» относится к множеству query heads. Key/value head при этом одна.
...
Query heads делятся на группы. Все heads внутри группы используют общие K/V.
...
## Shapes: место, где перестаёт работать интуиция
...
Но физически материализовывать четыре копии cache не нужно. Kernel может
переиспользовать один KV head для нескольких query heads. Иначе смысл экономии
частично потеряется.
