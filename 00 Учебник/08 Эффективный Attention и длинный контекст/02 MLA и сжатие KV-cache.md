...
# Multi-head Latent Attention: хранить не K/V, а их компактный источник

> [!abstract] Что нужно понять
> GQA уменьшает число явных K/V heads. MLA делает другой шаг: каждый token
> записывается в низкоразмерный latent, из которого можно восстановить
> head-specific content keys и values. На decode хранится latent и небольшая
> positional часть. Экономия возникает благодаря low-rank factorization и
> переносу части projections из cache path в query/output path.
...
Для hidden state токена $h_t\in\mathbb R^d$ обычный attention создаёт:

$$
q_t=W_Qh_t,\qquad k_t=W_Kh_t,\qquad v_t=W_Vh_t.
$$

После разделения по heads:
...
Для прошлого токена query больше не нужен: новый query появится на текущем
шаге. Но $k_t$ и $v_t$ будут нужны каждому будущему token, поэтому они попадают
в cache.
...
На один layer и token cache width равна:

$$
n_h(d_h+d_v).
$$

MLA спрашивает: обязаны ли мы хранить уже развёрнутые head-specific vectors,
если все они получены линейным преобразованием одного $h_t$?

## Шаг 1. Low-rank bottleneck для K и V

Сначала hidden state сжимается:
...
Буква $D$ означает down-projection. Из latent восстанавливаются content parts:
...
Это factorization исходных projections:
...
Если intermediate rank $d_c$ мал, cache хранит $c_t^{KV}$ вместо полных K/V.
...
Обычный MHA хранит на token/layer:
...
elements до учёта небольшой positional части. В этом игрушечном примере
compression ratio равна $16\times$.

Фактические размеры конкретной DeepSeek model нужно брать из config, а не из
этого примера. DeepSeek-V2 report сообщает 93.3% reduction KV-cache относительно
сравниваемой архитектуры; это результат конкретной конфигурации, не универсальная
константа MLA.

## Шаг 2. Queries тоже можно факторизовать

В полной MLA query projection также может проходить через low-rank bottleneck:
...
На cache это напрямую не влияет: queries прошлых tokens не сохраняются.
Query compression уменьшает параметры/compute projection и является отдельной
частью architecture.

В официальном DeepSeek-V3 reference code это видно по полям:
...
Название LoRA здесь описывает low-rank factorization внутри базовой
architecture; это не parameter-efficient fine-tuning adapters.
...
`x2.png`; MoE разбирается в следующей главе.*
