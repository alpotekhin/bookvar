...
вычисляется по нескольким ответам на один запрос. Источник: Shao et al., 2024,
[DeepSeekMath](https://arxiv.org/abs/2402.03300).*

```mermaid
flowchart LR
    Q["запрос q"] --> G["G ответов из π_old"]
    G --> R["награды r₁…r_G"]
    R --> A["относительные преимущества"]
    G --> L["вероятности токенов в π_old, π_θ, π_ref"]
    A --> U["ограниченное обновление стратегии"]
    L --> U
```
...
*Полная многоэтапная схема. Источник: DeepSeek-AI,
[DeepSeek-R1](https://arxiv.org/abs/2501.12948), Figure 1.*

```mermaid
flowchart TD
    B["DeepSeek-V3-Base"] --> C["начальные данные рассуждений и SFT"]
    C --> RR["RL на задачах рассуждения"]
    RR --> RS["отбор успешных траекторий"]
    RS --> MIX["добавление обычных диалоговых данных"]
    MIX --> SFT["повторный SFT"]
    SFT --> ALL["финальный RL: рассуждение, предпочтения и безопасность"]
    B -. "отдельный опыт только с RL" .-> Z["R1-Zero"]
```
